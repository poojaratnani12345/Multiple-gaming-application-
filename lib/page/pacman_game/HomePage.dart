...
Future<void> saveScoreToFirestore(int score) async {
  try {
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      if (!snapshot.exists) {
        // If not, create a new document with the initial score
        transaction.set(userDoc, {'packman_game_high_score': score});
      } else {
        // If it exists, get the current high score
        int currentHighScore = snapshot['packman_game_high_score'] ?? 0;
        
        // Update the high score if the new score is higher
        if (score > currentHighScore) {
          transaction.update(userDoc, {'packman_game_high_score': score});
        }
      }
    });
