...
Future<void> updateHighScore(int newScore) async {
  ...
  if (currentUser != null) {
    // Update the high score in Firestore if the new score is higher than the existing score
    ...
    if (userScoreDoc.exists && userScoreDoc.data() != null) {
      int currentHighScore = userScoreDoc['2048_high_score'] ?? 0;

      if (newScore > currentHighScore) {
        await FirebaseFirestore.instance.collection('users').doc(currentUser.uid).set({
          '2048_high_score': newScore,
    ...
    */
    ...
    if (played) {
      ...
      if (gameover) {
        ...
        // Call updateHighScore with the final score
        await updateHighScore(score);  // Pass the current score variable
      }
